...
func (h *Handlers) GetMyTenderwindow() func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if tender.IdStatus == 3{
			h.handleError(w, "В доступе отказано,тендер завершен", fmt.Errorf("Conflict, status"), 409)
			return
		}
		if tender.IdStatus == 4{
			h.handleError(w, "В доступе отказано,тендер отменен", fmt.Errorf("Conflict, status"), 409)
			return
		}
